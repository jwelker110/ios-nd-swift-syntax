...
var emptyDict = Dictionary<String,Bool>()
...
var dict = ["Anchovies": false, "Coconut": false, "Cilantro": false, "Liver": false]
...
let pet = presidentialPetsDict.removeValue(forKey: "George Bush")
if let pet = pet {
    presidentialPetsDict.updateValue(pet, forKey: "George W. Bush")
}
...
let releaseCount = studioAlbums.filter( { (key: String, value: Int) -> Bool in
    return key.contains("Led Zeppelin")
}).count
