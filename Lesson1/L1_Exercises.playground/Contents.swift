...
let yourSentence = "The \(nounArray[0]) met the \(nounArray[4])."
let yourSillySentence = "My \(nounArray[5]) flew to the \(nounArray[2])"
//: ### Exercise 2
...
let howManyCharacters = "How much wood could a woodchuck chuck if a woodchuck could chuck wood?".characters.count
//: ### Exercise 4
...
let gStringChars = gString.characters
var count = 0
for c in gStringChars {
    if c == "g" || c == "G" {
        count += 1
    }
}
...
print("Josie has $\(Double(numOfPennies) / 100)")
