...
var word = "bologna"
let letter: Character = word.remove(at: word.index(before: word.endIndex))
print(letter)
...
func combineLastCharacters(_ strings: [String]) -> String {
    var lastChars = ""
    for word in strings {
        lastChars.append(word[word.index(before: word.endIndex)])
    }
    return lastChars
}
...
func isOnlyDigits(_ str:String) -> Bool {
...
func cleanWords(_ dirtyWords: [String]) -> [String] {
    var clean: [String] = [];
...
class MovieArchive {
    ...
    func filterByDirector(_ director: String, _ movieDirectors: [String:String]) -> [String] {
        var movies: [String] = [];
        for (m, d) in movieDirectors {
            if d == director {
                movies.append(m)
            }
        }
...
let ma = MovieArchive()
print(ma.filterByDirector("Christopher Nolan", movies))
